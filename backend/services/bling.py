...
def _salvar_token(data: dict, db: Session):
    ...
    if token_db:
        ...
        token_db = TokenBling(access_token=data["access_token"], refresh_token=data["refresh_token"], expires_at=expires_at)
        db.add(token_db)
    db.commit()
...
async def sincronizar_pedidos(db: Session, dias: int = 30) -> dict:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        while True:
            resp = await client.get(f"{BLING_BASE_URL}/pedidos/vendas", headers=headers, params={"pagina": pagina, "limite": 100, "dataInicial": data_inicio})
            if resp.status_code != 200:
                break
            data = resp.json()
            ...
            for pedido in pedidos:
                ...
                elif resultado == "atualizado": atualizados += 1
...
def _processar_pedido(pedido: dict, db: Session) -> str:
    ...
    num_itens = sum(int(item.get("quantidade", 0)) for item in (pedido.get("itens") or []))
    ...
    if venda_db:
        ...
        nova = Venda(bling_pedido_id=bling_id, vendedora_nome=vendedora_nome, cliente_nome=cliente_nome, cliente_id_bling=cliente_id, valor_total=valor_total, num_itens=num_itens, data_venda=data_venda, situacao=situacao)
        db.add(nova)
        return "novo"
